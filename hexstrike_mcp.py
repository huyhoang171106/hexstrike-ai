...
""",
    ...
    # ============================================================================
    # CTF HARD CHALLENGE SOLVER TOOLS (v6.0)
    # ============================================================================

    @mcp.tool()
    def ctf_analyze_hard(category: str = "pwn", description: str = "", file_data: str = "") -> Dict[str, Any]:
        """
        Analyze a hard CTF challenge and get suggested techniques.

        Args:
            category: Challenge category (pwn, crypto, forensics, re, web, stego)
            description: Challenge description or hints
            file_data: Any strings/metadata from the challenge file

        Returns:
            JSON analysis with suggested techniques and tool availability
        """
        data = {
            "category": category,
        ...
        logger.info(f"{HexStrikeColors.RUBY}🔬 Analyzing hard CTF challenge: {category}{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/ctf/analyze-hard", data)
        return result

    @mcp.tool()
    def ctf_auto_solve(category: str = "pwn", target: str = "", difficulty: str = "medium") -> Dict[str, Any]:
        """
        Attempt to automatically solve a CTF challenge.

        Args:
            category: Challenge category (pwn, crypto, forensics, etc.)
            target: Target file or service
            difficulty: Challenge difficulty (easy, medium, hard)

        Returns:
            JSON result with solve status and flag if found
        """
        data = {
            "category": category,
            "target": target,
        ...
        logger.info(f"{HexStrikeColors.RUBY}🤖 Attempting CTF auto-solve: {category} - {difficulty}{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/ctf/auto-solve", data)
        return result

    @mcp.tool()
    def ctf_get_hint(category: str = "pwn", level: int = 1, techniques_tried: str = "") -> Dict[str, Any]:
        """
        Get a progressive hint for a CTF challenge.

        Args:
            category: Challenge category
            level: Hint level (1=direction, 2=method, 3=technique)
            techniques_tried: Comma-separated list of techniques already tried

        Returns:
            Hint text and next steps
        """
        techniques = [t.strip() for t in techniques_tried.split(",") if t.strip()]
        data = {
            "category": category,
            "level": level,
            "techniques_tried": techniques
        }
        logger.info(f"{HexStrikeColors.RUBY}💡 Getting CTF hint: {category} - level {level}{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/ctf/get-hint", data)
        return result

    @mcp.tool()
    def ctf_check_tools(category: str = "pwn") -> Dict[str, Any]:
        """
        Check which tools are available for a category.

        Args:
            category: Tool category (pwn, crypto, forensics, re, web, stego)

        Returns:
            List of available and missing tools
        """
        logger.info(f"{HexStrikeColors.RUBY}🔧 Checking CTF tools for: {category}{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_get(f"api/ctf/tool-check?category={category}")
        return result

    @mcp.tool()
    def ctf_get_patterns(category: str = "pwn") -> Dict[str, Any]:
        """
        Get technique patterns for a CTF category.

        Args:
            category: Category to get patterns for (pwn, crypto, forensics, re, web, stego)

        Returns:
            List of techniques with descriptions
        """
        logger.info(f"{HexStrikeColors.RUBY}📚 Getting CTF patterns for: {category}{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_get(f"api/ctf/patterns?category={category}")
        return result

    @mcp.tool()
    def ctf_search_patterns(keyword: str = "") -> Dict[str, Any]:
        """
        Search technique patterns by keyword.
        ...
        """
        logger.info(f"{HexStrikeColors.RUBY}🔍 Searching CTF patterns: {keyword}{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_get(f"api/ctf/search-patterns?keyword={keyword}")
        return result
